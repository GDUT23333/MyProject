...
<head>
    ...
    <title>注册页面</title>
    <style>
        #email,#password,#c_password,#name,#phone,#nameTwo{
            width: 150px;
            height: 30px;
    ...
    <script>
        window.onload = function () {
            ...
            document.getElementById("form").onsubmit = function () {
                var flag =  checkPassword()&&comfirmPassword()&&checkName()&&checkTwoName()&&checkPhone();
                if(flag){
                    loadPhone();
                }
                return false;
            ...
            function loadPhone() {
                ...
                xhttp.open("GET", '/checkUserServlet?email='+email+'&phone='+phone+'', true);
                xhttp.send();
                xhttp.onreadystatechange = function () {
                    if (this.readyState == 4 && this.status == 200) {
                        if (xhttp.responseText == "未被注册") {
                            var email = document.getElementById("email").value;
                            ...
                            var user = {"email":email,"password":password,"name":name,"nameTwo":nameTwo,"phone":phone,"sex":sex};
                            var data = JSON.stringify(user);
                            location.href = "/userRegisterServlet?data=" + encodeURIComponent(data);
                        } else {
...
<div>
    ...
    <span style="font-size: 20px;height: 30px">用户注册</span>
</div>
<div>
...
<form action="#" id="form" method="post">
    <table  style="margin-top: 40px;margin-left: 500px">
        <tr><td class="right"><span>登录邮箱:</span></td>
            <td class="left"><input name="email" id="email" type="email">
            </td>
        </tr>
        <tr>
